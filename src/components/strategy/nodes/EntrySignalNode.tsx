...
const EntrySignalNode = ({ data, id }: EntrySignalNodeProps) => {
  ...
  const conditionCount = hasConditions ? countConditions(conditions[0]) : 0;
  ...
  return (
    <div className="px-3 py-2 rounded-md shadow-sm bg-white dark:bg-gray-800 border border-border">
      ...
      <div className="flex items-center mb-1.5">
        <ArrowUpRight className="h-4 w-4 text-emerald-600 dark:text-emerald-400 mr-1.5" />
        <div className="font-medium text-xs">{data.label || "Entry Signal"}</div>
      </div>
